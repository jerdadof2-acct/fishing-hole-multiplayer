...
/**
 * Trailing V-shaped wake ripples that follow a bobber during reel-in and fights.
...
export class BobberWake {
     ...
     * @param {boolean} active — reel-in, fight, or landing
    ...
    update(delta, bobberPos, fighting, getWaterHeight, shaderRipple = null) {
        if (!fighting) {
            this.active = false;
            this.hasLastPos = false;
            // Stop creating new wake pieces, but allow existing ones to fade.
        } else {
            this.active = true;
        }
        ...
        if (fighting) {
            ...
            if (this.hasLastPos) {
                this.distanceSinceSpawn += moved;
            }

            const speed = moved / Math.max(delta, 0.001);
            const movingFastEnough = speed > 0.12;
            ...
            if (
                this.hasLastPos &&
                movingFastEnough &&
                this.distanceSinceSpawn >= spawnDistance
            ) {
                const heading = Math.atan2(dx, dz);

                const behind = 0.18 + Math.min(0.35, speed * 0.05);
                const armBack = 0.22 + Math.min(0.28, speed * 0.045);
                const armSide = 0.055 + Math.min(0.09, speed * 0.012);
                ...
                for (const side of [-1, 1]) {
                    ...
                    const outwardX = Math.cos(heading) * side;
                    const outwardZ = -Math.sin(heading) * side;

                    this._spawn(
                        ax,
                        az,
                        ay,
                        0.62 + Math.min(0.38, speed * 0.06),
                        0.075,
                        -armHeading,
                        1.35,
                        0.34,
        ...
        for (const mesh of this.pool) {
            ...
            mesh.position.x += (mesh.userData.driftX || 0) * delta;
            mesh.position.z += (mesh.userData.driftZ || 0) * delta;
            ...
            const fade = 1 - THREE.MathUtils.smoothstep(t, 0.15, 1.0);
            mesh.material.opacity =
                (mesh.userData.startOpacity || 0.34) * fade;
        }
    }
}
